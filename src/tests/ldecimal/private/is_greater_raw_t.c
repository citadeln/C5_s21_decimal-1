...
START_TEST(t1) {
  // arrange
  const s21_ldecimal num1 = {.bits = {3}, .exp = 0};
  const s21_ldecimal num2 = {.bits = {2}, .exp = 0};

  int need_result = true;

  // act
  int result = s21_ldec_is_greater_raw(&num1, &num2);
...
START_TEST(exp_not_equal1) {
  ...
  // act
  int result = s21_ldec_is_greater_raw(&value_1, &value_2);
...
TCase *is_greater_raw_tc(void) {
  TCase *tc = tcase_create("s21_ldec_is_greater_raw");
